...
$teaser = static function (array $s, array $extraLinks = [], string $extraHeading = '') use ($t, $fancy, $ed, $editing): string {
    ...
    $out = '<li class="group/li"><section class="nc-teaser majors-section" data-section="' . (int) $s['id'] . '"' . ($s['shared'] ? ' data-shared="1"' : '') . $ed->scope($s) . '>'
        ...
        . '<h2 class="font-display [&_*]:!font-display font-semibold [&_*]:!font-semibold text-xl sm:text-2xl/[1.85rem] md:group-data-[max-cols=2]/teaser-grid:text-size-3xl md:group-has-[>:nth-child(2):last-child]/teaser-grid:text-size-3xl md:group-has-[>:only-child]/teaser-grid:text-size-3xl"' . $ed->text('headline', $s['headline']) . '>' . $ed->show($s['headline'], 'Click to write the headline') . '</h2>';
    if ($s['body'] !== '' || $editing) {
