...
<?php foreach (array_merge($links, [['text' => '', 'href' => '']]) as $l): ?>
			<div class="ma-link-row"><input type="text" name="links[text][]" value="<?= $t->e($l['text'] ?? '') ?>" placeholder="Link text" maxlength="200"><input type="text" name="links[href][]" value="<?= $t->e($l['href'] ?? '') ?>" placeholder="https://…" maxlength="500"><button type="button" class="ma-link-remove" title="Remove">×</button></div>
<?php endforeach; ?>
